        ...
        for ea in etab_annees:
            ...
            for eac in eac_list:
                ...
                config_classes.append({
                    'name': cls.nom if cls else '-',
                    'id_classe': cls.id_classe if cls else eac.classe_id,
                    'cycle': cls.cycle.nom if cls and cls.cycle else '',
                    'section': sec.nom if sec else None,
                    'groupe': eac.groupe,
                    'nb_cours': nb_cours,
                    'has_cours': has_cours,
                })
        ...
        if etab_annee:
            ...
            for cc in classes_config:
                ...
                classes_detail.append({
                    ...
                    'cycle_nom': cls.cycle.nom if cls and cls.cycle else '-',
                    'section_nom': f"{sec.code} - {sec.nom}" if sec else '-',
                    'groupe': cc.groupe or '',
                })
        ...
        try:
            with conn.cursor() as cur:
                ...
                for cfg in (parent_configs if not _is_cycle_sans_periodes else []):
                    ...
                    # Find child period config IDs for this parent via positional mapping
                    child_cfg_ids = trimester_children_map.get(config_id, [])
...
def get_deliberated_classes(request):
    ...
    try:
        ...
        for eac in eac_list:
            ...
            section_name = sec.nom if sec else None
